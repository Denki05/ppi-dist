...
<nav class="breadcrumb bg-white push">
  ...
  <a class="breadcrumb-item" href="{{ route('superuser.gudang.purchase_order_spk.step', $purchase_order->id) }}">{{ $purchase_order->code }}</a>
  ...
  <div class="block-content">
    ...
    <form id="frmCreate" action="#" data-type="POST" enctype="multipart/form-data">
      ...
      <div class="form-group row pt-30">
        <div class="col-md-6">
          <a href="{{ route('superuser.gudang.purchase_order_spk.step', $purchase_order->id) }}">
  ...
  $(document).ready(function () {
    ...
    $(document).on('submit','#frmCreate',function(e){
      ...
      if(confirm("Apakah anda yakin ingin menambakan Product ke PO ?")){
        let _form = $('#frmCreate');
        $.ajax({
          url : '{{route('superuser.gudang.purchase_order_spk.detail.store', $purchase_order->id)}}',
          method : "POST",
            ...
            if(resp.IsError == true){
              ...
              Swal.fire(
                ...
                'success'
                  ...
                  document.location.href = '{{ route('superuser.gudang.purchase_order_spk.step', $purchase_order->id) }}';
              })
    ...
    function loadProduct(param){
      $.ajax({
        url : '{{route('superuser.gudang.purchase_order_spk.detail.get_product')}}',
        method : "GET",
        data : param,
        dataType : "JSON",
        success : function(resp){
