...
<div>
  <h2 style="text-align: center; margin-bottom: 10px;"><u>SJ RECEIVING - KOMPLAIN</u></h2>
  ...
  <div class="row-float clearfix" style="margin-bottom: 15px;">
    ...
    <div class="column-float" style="width: 50%;">
      <table class="info">
        <tr>
            <td style="width: 35%;">Kode</td><td style="width: 5%;">:</td><td>{{ $code }}</td>
        </tr>
        <tr>
            <td style="width: 35%;">Tanggal</td><td style="width: 5%;">:</td><td>{{ date('d-m-Y', strtotime($result->tanggal)) }}</td>
        </tr>
        <tr>
  ...
  <table class="table-data">
    ...
    <tbody>
      @foreach ($result->details as $index => $row)
        @php
            $jumlah = $row->qty / $row->product_pack->packaging->pack_value;
        @endphp
        <tr>
          <td>{{ $loop->iteration }}</td>
          <td class="text-left">{{ $row->product_pack->code }} - {{ $row->product_pack->name }}</td>
          <td>{{ $row->qty }}</td>
