...
<div class="block">
  <div class="block-header block-header-default">
    <h3 class="block-title">Edit Min Stock</h3>
  </div>
  <div class="block-content">
    <form class="ajax" data-action="{{ route('superuser.master.product.min_stock.update', [$product->id, $min_stock->id]) }}" data-type="POST" enctype="multipart/form-data">
      ...
      <div class="form-group row">
        ...
        <div class="col-md-7">
          <select class="js-select2 form-control" id="warehouse" name="warehouse" data-placeholder="Select Warehouse">
            ...
            <option value="{{ $warehouse->id }}" {{ ($min_stock->warehouse_id == $warehouse->id) ? 'selected' : '' }}>{{ $warehouse->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
      <div class="form-group row">
        ...
        <div class="col-md-4">
          <input type="number" class="form-control" id="quantity" name="quantity" min="0" value="{{ $min_stock->quantity }}" step="0.0001">
        </div>
        <div class="col-md-3">
          <select class="js-select2 form-control" id="unit" name="unit" data-placeholder="Unit">
            <option></option>
            @foreach($units as $unit)
            <option value="{{ $unit->id }}" {{ ($min_stock->unit_id == $unit->id) ? 'selected' : '' }}>{{ $unit->abbreviation }} / {{ $unit->name }}</option>
            @endforeach
          </select>
        </div>
      </div>
