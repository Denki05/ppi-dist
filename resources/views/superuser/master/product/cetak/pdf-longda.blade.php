  ...
  <table style="width: 100%;" class="table-data">
    ...
    <thead>
      <tr>
        ...
        <th class="text-center border-full">Brand</th>
        <th class="text-center border-full">Sub Brand</th>
        @php
    ...
    <tbody>
      ...
      @endphp
        <tr>
          <td class="text-center border-full" style="background-color: #ccc"><strong>{{ ('type' == $groupBy) ? $row->group_value : '' }}</strong> {{ '' != $typeText ? 'Kode' : 'Group - Kode' }}</td>
          <td class="text-center border-full" style="background-color: #ccc">Nama</td>
          <td class="text-center border-full" style="background-color: #ccc">Searah <strong>{{ ('searah' == $groupBy) ? $row->group_value : '' }}</strong></td>
          @php
            if ($request->jenisReport == 'pl') {
          @endphp
      ...
      <tr>
        ...
        <td>@php echo $row->sub_brand @endphp</td>
        @php
          if ($request->jenisReport == 'pl') {
        @endphp
