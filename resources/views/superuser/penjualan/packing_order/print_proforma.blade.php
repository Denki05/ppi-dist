<?php
    ...
    $idr_total = 0; 
    $code = explode("PRE", $result->code ?? null);
    $date = date('my',strtotime($result->created_at));
    $code = 'PRE-'.$date.'-'.$code[1];
?>
<!DOCTYPE html>
<html>
<head>
  ...
  <style type="text/css">
    ...
    .page-number:before {
      content: '<?= $code." | ".$result->customer->name ?? null; ?>' " | Page " counter(page);
    }
  </style>
</head>
<body>
    <div class="row-float" style="margin-bottom: 20px !important;">
      ...
      <div class="column-float" style="width: 55%;margin-top: 15px;">
        ...
        <h3 style="text-align: center;margin: 0;padding: 0;margin-bottom: 5 !important;padding-bottom: 0 !important;">{{date('d-m-Y',strtotime($result->created_at))}}</h3>
          ...
          <table class="table borderless info" style="width: 100%;">
            <tbody>
              ...
              <tr>
                ...
                <td style="width: 2% !important;">:</td>
                <td style="width: 63% !important;">{{$result->customer->owner_name ?? ''}}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table class="table borderless info" style="width: 100%">
            <tbody>
              <tr>
                ...
                <td style="width: 2% !important;">:</td>
                <td style="width: 63% !important;">{{$result->customer->phone ?? ''}}</td>
              </tr>
              <tr>
                <td style="width: 35% !important;"><strong>NIK/NPWP</strong></td>
                <td style="width: 2% !important;">:</td>
                <td style="width: 63% !important;">{{$result->customer->npwp ?? ''}}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <table class="table borderless info" style="width: 100%;">
          <tbody>
            <tr>
              ...
              <td style="width: 2% !important;">:</td>
              <td style="width: 81.5% !important;">{{$result->customer->address ?? ''}}</td>
            </tr>
          </tbody>
        </table>
      </div>
          ...
          <table class="table borderless info" style="width: 100%;">
            <tbody>
              <tr>
                ...
                <td style="width: 2% !important;">:</td>
                <td style="width: 63% !important;">{{$result->ekspedisi->name ?? ''}}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <table class="table borderless info" style="width: 100%">
            <tbody>
              <tr>
                <td style="width: 35% !important;"><strong>Telepon</strong></td>
                <td style="width: 2% !important;">:</td>
                <td style="width: 63% !important;">{{$result->customer_other_address->phone ?? ''}}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <table class="table borderless info" style="width: 100%;">
          <tbody>
            <tr>
              ...
              <td style="width: 2% !important;">:</td>
              <td style="width: 81.5% !important;">{{$result->customer_other_address->address ?? ''}}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <table class="table-data" style="width: 100%;">
        ...
        <tbody>
          @foreach($result->do_detail as $index => $row)
            <?php
              $harga = ceil($result->idr_rate * $row->price);
              $disc_cash = ceil($result->idr_rate * ($row->total_disc/$row->qty));
    ...
    @if($result->type_transaction == 1 || $result->type_transaction == 3)
        ...
        <div class="row">
          <table style="width: 100%;">
            <tbody>
              ...
              @if($result->do_cost->discount_1 > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Discount 1 (%)</td>
                <td style="width: 40%;text-align: left;">: {{$result->do_cost->discount_1}}</td>
              </tr>
              @endif
              @if($result->do_cost->discount_2 > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Discount 2 (%)</td>
                <td style="width: 40%;text-align: left;">: {{$result->do_cost->discount_2}}</td>
              </tr>
              @endif
              @if($result->do_cost->discount_idr > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Discount (IDR)</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->discount_idr ?? 0,0,',','.')}}</td>
              </tr>
              @endif
              @if($result->do_cost->total_discount_idr > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Total Discount (IDR)</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->total_discount_idr ?? 0,0,',','.')}}</td>
              </tr>
              @endif
              @if($result->do_cost->ppn > 0)
              <tr>
                <td style="width: 60%;text-align: right;">PPN</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->ppn ?? 0 ,0,',','.')}}</td>
              </tr>
              @endif
              @if($result->do_cost->voucher_idr > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Voucher (IDR)</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->voucher_idr ?? 0,0,',','.')}}</td>
              </tr>
              @endif
              @if($result->do_cost->cashback_idr > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Cashback (IDR)</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->cashback_idr ?? 0,0,',','.')}}</td>
              </tr>
              @endif
              @if($result->do_cost->purchase_total_idr > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Purchase Total (IDR)</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->purchase_total_idr ?? 0,0,',','.')}}</td>
              </tr>
              @endif
              @if($result->do_cost->delivery_cost_idr > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Delivery Cost (IDR)</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->delivery_cost_idr ?? 0,0,',','.')}}</td>
              </tr>
              @endif
              @if($result->do_cost->other_cost_idr > 0)
              <tr>
                <td style="width: 60%;text-align: right;">Other Cost (IDR)</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->other_cost_idr ?? 0,0,',','.')}}</td>
              </tr>
              @endif
              <tr>
                <td style="width: 60%;text-align: right;">Grand Total (IDR)</td>
                <td style="width: 40%;text-align: left;">: {{number_format($result->do_cost->grand_total_idr ?? 0,0,',','.')}}</td>
              </tr>
            </tbody>
          </table>
        </div>
