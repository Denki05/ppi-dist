    ...
    public function collection()
    {
        $model = PurchaseOrder::leftJoin('purchase_order_detail', 'purchase_order_detail.po_id', '=', 'purchase_order.id')
                    ...
                    ->when(!empty($this->start_date), function ($query) {
                        $query->whereDate('purchase_order.created_at', '>=', $this->start_date);
                    })
                    ->when(!empty($this->end_date), function ($query) {
                        $query->whereDate('purchase_order.created_at', '<=', $this->end_date);
                    })
                    ->select(
                        ...
                        'purchase_order_detail.note_produksi AS note',
