...
class SalesOrderKontrakController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            try{
                ...
                $validator = Validator::make($request->all(), [
                    'code' => 'required|string|unique:penjualan_so_kontrak,code',
    ...
    public function edit(Request $request, $id)
    {
        ...
        $data['product'] = ProductPack::get()->unique('id');
    ...
    public function update_revisi(Request $request, $id)
    {
        if ($request->ajax()) {
            ...
            try{
                ...
                $cal_range = $sales_kontrak->contract_range * date('t');
                $start_date = Carbon::parse($sales_kontrak->created_at);
                $end_date = $sales_kontrak->created_at->addDays($cal_range);
                
                if(empty($log_kontrak)){
                    $errors[] = 'Pengambilan Qty Kontrak belum ada!';
                }

                if($sales_kontrak){
                    if($sales_kontrak->item->qty != $log_kontrak->sum('qty_worked')){
    ...
    public function print_log()
    {
        ...
        $my_report = "C:\\xampp\\htdocs\\ppi-dist\public\\cr\\report\\operasional\\log_kontrak\\log_kontrak.rpt"; 
