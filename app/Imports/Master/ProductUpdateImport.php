    ...
    public function collection(Collection $rows)
    {
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                // Validate the ratio format, allow decimals with the updated regex
                if (!empty($row['ratio']) && preg_match('/^(\d+(,\d+)?(\+(\d+(,\d+)?))*)+:(\d+(,\d+)?(\+(\d+(,\d+)?))*)$/', $row['ratio'])) {
                    $product->ratio = $row['ratio']; // Store as string (e.g., "12.5:25")
                    $product->save();
                    $collect_success[] = "Updated Product: {$product->code} - {$product->name}, Ratio: {$row['ratio']}";
                } else {
                    $collect_error[] = "Invalid or missing ratio for product ID " . $product->name;
                }
            }
